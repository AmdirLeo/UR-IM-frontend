      ...
      <div className="flex-1 overflow-y-auto bg-primary">
        {loading ? (
          ...
          <div className="flex flex-col">
              ...
              {members.slice(0, 19).map(member => (
                ...
                >
                  <div className="w-10 h-10 bg-gray-200 rounded flex items-center justify-center overflow-hidden mb-1 hover:opacity-80 transition-opacity relative">
                    ...
                    {member.role === 'owner' && (
                      <div className="absolute bottom-0 right-0 bg-yellow-500 text-[8px] text-white px-[2px] rounded-tl">群主</div>
                    )}
                  </div>
            ...
            <div className="bg-panel px-4 py-3 flex flex-col border-b border-primary">
              <span className="text-sm text-secondary mb-1">群公告</span>
              <span className="text-base text-secondary">{groupInfo?.latest_announcement?.content || '群主未设置'}</span>
            </div>
            ...
            <div className="bg-panel px-4 py-4 flex items-center justify-between cursor-pointer hover:bg-hover transition-colors">
