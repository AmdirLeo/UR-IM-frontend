...
export const MainLayout: React.FC<MainLayoutProps> = ({ currentUserId, username, token, onLogout }) => {
  ...
  const handleSendMessage = async (contactId: number) => {
    try {
      // First, get or create the actual conversation_id for this friend
      const response = await chatApi.getDirectConversation(contactId);
      if (response.code === 200 && response.data?.conversation_id) {
        setActiveChatId(response.data.conversation_id);

        // Also ensure conversations are re-loaded so the new/existing conversation shows up in the list
        await loadConversations();

        setPreviousView(activeView);
        setActiveView('messages');
      } else {
        console.error("Failed to get direct conversation ID:", response);
        // Fallback behavior if needed, or simply alert
